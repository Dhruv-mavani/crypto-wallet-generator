...
export const EthWallet = ({ mnemonic, network }: EthWalletProps) => {
    ...
    const addWallet = () => {
        ...
        setWallets([...wallets, { address: child.address, balance: 0, privateKey: child.privateKey }]);
    ...
    const refreshBalance = async (address: string) => {
        ...
        const balanceInEth = formatEther(data.result);
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 sm:p-6 rounded-2xl shadow-xl transition-colors duration-300">
            ...
            <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                {wallets.map((wallet, i) => (
                    <EthWalletRow key={i} wallet={wallet} i={i} refreshBalance={refreshBalance} network={network} deleteWallet={deleteWallet} />
                ))}
                {wallets.length === 0 && (
...
const EthWalletRow = ({ wallet, i, refreshBalance, network, deleteWallet }: { wallet: any, i: number, refreshBalance: any, network: string, deleteWallet: any }) => {
    ...
    const handleSend = async () => {
        ...
        if (numAmount > wallet.balance) {
            setError(`Insufficient balance. You only have ${wallet.balance.toFixed(4)} ETH.`);
            return;
        }
        ...
        if (toAddress.toLowerCase() === wallet.address.toLowerCase()) {
            setError("You cannot send ETH to yourself.");
            return;
        }

        setIsSending(true);
        const result = await signEthToken(wallet.privateKey, toAddress, numAmount, network);
        if (result.success) {
        ...
        <div className="bg-slate-50 dark:bg-black/40 p-3 sm:p-4 rounded-xl border border-slate-200 dark:border-slate-800 flex flex-col gap-3 transition-colors duration-300">
            ...
            <div className="flex flex-col gap-1">
                <div className="flex justify-between items-center">
                    <span className="text-xs text-slate-500 dark:text-slate-400 font-semibold">Private Key:</span>
                    <button
                        ...
                        {isScanning ? (
                            ...
                            <div className="flex flex-col gap-4">
                                ...
                                <div className="flex flex-col gap-1">
                                    ...
                                    />
                                    <span className="text-xs text-slate-500">Balance: {wallet.balance.toFixed(4)} ETH</span>
                                </div>
